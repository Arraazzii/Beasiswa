<?php  
include '../lib/config.php';

$no_bea = $_POST['no_bea'];
$nama_bea = $_POST['nama_bea'];
$tahun_beasiswa = $_POST['tahun_beasiswa'];
$jenis_bea = $_POST['jenis_bea'];
$total_dana_bea = $_POST['total_dana_bea'];
$jumlah_bea = $_POST['jumlah_bea'];
$ket_bea = $_POST['ket_bea'];


$query_update="UPDATE info_beasiswa SET  st = 'N' ";
mysql_query($query_update);

$query="INSERT INTO info_beasiswa SET no_beasiswa='$no_bea', nama_beasiswa='$nama_bea', tahun='$tahun_beasiswa', jenjang='$jenis_bea', total_dana='$total_dana_bea', jumlah_beasiswa='$jumlah_bea', keterangan='$ket_bea' , st = 'Y' ";
mysql_query($query);

$pengu_beasiswa = $_POST['pengu_beasiswa'];
$wtf1 = $_POST['wtf1'];
$seleksi_admin = $_POST['seleksi_admin'];
$spd2 = $_POST['spd2'];
$pengu_selad = $_POST['pengu_selad'];
$spd3 = $_POST['spd3'];
$seleksi_wawan = $_POST['seleksi_wawan'];
$spd4 = $_POST['spd4'];
$pengu_penbe = $_POST['pengu_penbe'];
$spd5 = $_POST['spd5'];
$daftar_ulang = $_POST['daftar_ulang'];
$spd6 = $_POST['spd6'];
$pene_dabe = $_POST['pene_dabe'];
$spd7 = $_POST['spd7'];

$query1="INSERT INTO jadwal_beasiswa SET no_beasiswa='$no_bea',pengumuman='$pengu_beasiswa', spd1='$wtf1',
seleksi='$seleksi_admin',
spd2='$spd2',
pengumuman_seleksi='$pengu_selad',
spd3='$spd3',
seleksi_wawancara='$seleksi_wawan',
spd4='$spd4',
pengumuman_penerima='$pengu_penbe',
spd5='$spd5',
daftar_ulang='$daftar_ulang',
spd6='$spd6',
penerimaan_dana='$pene_dabe',
spd7='$spd7'";
mysql_query($query1);


$bts_lulus = $_POST['bts_lulus'];
$batas_lulus_wawancara = $_POST['batas_lulus_wawancara'];

mysql_query("INSERT INTO nilai_minimum SET no_beasiswa = '$no_bea', seleksi_wawancara = '$batas_lulus_wawancara', seleksi_admin = '$bts_lulus'");


$nr_index0 = $_POST['nr_index0'];
$nr_spd0 = $_POST['nr_spd0'];
$po_nr_index0 = $_POST['po_nr_index0'];
$nr_index1 = $_POST['nr_index1'];
$nr_spd1 = $_POST['nr_spd1'];
$po_nr_index1 = $_POST['po_nr_index1'];
$nr_index2 = $_POST['nr_index2'];
$nr_spd2 = $_POST['nr_spd2'];
$po_nr_index2 = $_POST['po_nr_index2'];
$nr_index3 = $_POST['nr_index3'];
$nr_spd3 = $_POST['nr_spd3'];
$po_nr_index3 = $_POST['po_nr_index3'];

mysql_query("

INSERT INTO admis_nilai_raport SET no_beasiswa = '$no_bea', 
index0='$nr_index0',
spd0='$nr_spd0',
index1='$nr_index1',
spd1='$nr_spd1',
index2='$nr_index2',
spd2='$nr_spd2',
index3='$nr_index3',
spd3='$nr_spd3',
poin1='$po_nr_index0',
poin2='$po_nr_index1',
poin3='$po_nr_index2',
poin4='$po_nr_index3'

");


$jpalo_index0 = $_POST['jpalo_index0'];
$po_jpalo_index0 = $_POST['po_jpalo_index0'];
$jpalo_spd0 = $_POST['jpalo_spd0'];
$jpalo_index1 = $_POST['jpalo_index1'];
$jpalo_spd1 = $_POST['jpalo_spd1'];
$po_jpalo_index1 = $_POST['po_jpalo_index1'];
$jpalo_index2 = $_POST['jpalo_index2'];
$jpalo_spd2 = $_POST['jpalo_spd2'];
$po_jpalo_index2 = $_POST['po_jpalo_index2'];
$jpalo_index3 = $_POST['jpalo_index3'];
$jpalo_spd3 = $_POST['jpalo_spd3'];
$po_jpalo_index3 = $_POST['po_jpalo_index3'];

mysql_query("

INSERT INTO jpa SET no_beasiswa = '$no_bea', 
index0='$jpalo_index0',
spd0='$jpalo_spd0',
index1='$jpalo_index1',
spd1='$jpalo_spd1',
index2='$jpalo_index2',
spd2='$jpalo_spd2',
index3='$jpalo_index3',
spd3='$jpalo_spd3',
poin1='$po_jpalo_index0',
poin2='$po_jpalo_index1',
poin3='$po_jpalo_index2',
poin4='$po_jpalo_index3'
");

$jpakec_index0 = $_POST['jpakec_index0'];
$jpakec_spd0 = $_POST['jpakec_spd0'];
$po_jpakec_index0 = $_POST['po_jpakec_index0'];
$jpakec_index1 = $_POST['jpakec_index1'];
$jpakec_spd1 = $_POST['jpakec_spd1'];
$po_jpakec_index1 = $_POST['po_jpakec_index1'];
$jpakec_index2 = $_POST['jpakec_index2'];
$jpakec_spd2 = $_POST['jpakec_spd2'];
$po_jpakec_index2 = $_POST['po_jpakec_index2'];
$jpakec_index3 = $_POST['jpakec_index3'];
$jpakec_spd3 = $_POST['jpakec_spd3'];
$po_jpakec_index3 = $_POST['po_jpakec_index3'];

mysql_query("

INSERT INTO jpna SET no_beasiswa = '$no_bea', 
index0='$jpakec_index0',
spd0='$jpakec_spd0',
index1='$jpakec_index1',
spd1='$jpakec_spd1',
index2='$jpakec_index2',
spd2='$jpakec_spd2',
index3='$jpakec_index3',
spd3='$jpakec_spd3',
poin1='$po_jpakec_index0',
poin2='$po_jpakec_index1',
poin3='$po_jpakec_index2',
poin4='$po_jpakec_index3'
");




$ni_index0 = $_POST['ni_index0'];
$ni_spd0 = $_POST['ni_spd0'];
$po_ni_index0 = $_POST['po_ni_index0'];
$ni_index1 = $_POST['ni_index1'];
$ni_spd1 = $_POST['ni_spd1'];
$po_ni_index1 = $_POST['po_ni_index1'];
$ni_index2 = $_POST['ni_index2'];
$ni_spd2 = $_POST['ni_spd2'];
$po_ni_index2 = $_POST['po_ni_index2'];
$ni_index3 = $_POST['ni_index3'];
$ni_spd3 = $_POST['ni_spd3'];
$po_ni_index3 = $_POST['po_ni_index3'];

mysql_query("

INSERT INTO admis_nilai_ket SET no_beasiswa = '$no_bea', 
index0='$ni_index0',
spd0='$ni_spd0',
index1='$ni_index1',
spd1='$ni_spd1',
index2='$ni_index2',
spd2='$ni_spd2',
index3='$ni_index3',
spd3='$ni_spd3',
poin1='$po_ni_index0',
poin2='$po_ni_index1',
poin3='$po_ni_index2',
poin4='$po_ni_index3'
");


/*ipk*/
$ipk_index0 = $_POST['ipk_index0'];
$ipk_spd0 = $_POST['ipk_spd0'];
$po_ipk_index0 = $_POST['po_ipk_index0'];
$ipk_index1 = $_POST['ipk_index1'];
$ipk_spd1 = $_POST['ipk_spd1'];
$po_ipk_index1 = $_POST['po_ipk_index1'];
$ipk_index2 = $_POST['ipk_index2'];
$ipk_spd2 = $_POST['ipk_spd2'];
$po_ipk_index2 = $_POST['po_ipk_index2'];
$ipk_index3 = $_POST['ipk_index3'];
$ipk_spd3 = $_POST['ipk_spd3'];
$po_ipk_index3 = $_POST['po_ipk_index3'];

mysql_query("

INSERT INTO admis_nilai_ipk SET no_beasiswa = '$no_bea', 
index0='$ni_index0',
spd0='$ni_spd0',
index1='$ni_index1',
spd1='$ni_spd1',
index2='$ni_index2',
spd2='$ni_spd2',
index3='$ni_index3',
spd3='$ni_spd3',
poin1='$po_ni_index0',
poin2='$po_ni_index1',
poin3='$po_ni_index2',
poin4='$po_ni_index3'
");

$ipkket_index0 = $_POST['ipkket_index0'];
$ipkket_spd0 = $_POST['ipkket_spd0'];
$po_ipkket_index0 = $_POST['po_ipkket_index0'];
$ipkket_index1 = $_POST['ipkket_index1'];
$ipkket_spd1 = $_POST['ipkket_spd1'];
$po_ipkket_index1 = $_POST['po_ipkket_index1'];
$ipkket_index2 = $_POST['ipkket_index2'];
$ipkket_spd2 = $_POST['ipkket_spd2'];
$po_ipkket_index2 = $_POST['po_ipkket_index2'];
$ipkket_index3 = $_POST['ipkket_index3'];
$ipkket_spd3 = $_POST['ipkket_spd3'];
$po_ipkket_index3 = $_POST['po_ipkket_index3'];

mysql_query("
INSERT INTO admis_nilai_ipk_ket SET no_beasiswa = '$no_bea', 
index0='$ni_index0',
spd0='$ni_spd0',
index1='$ni_index1',
spd1='$ni_spd1',
index2='$ni_index2',
spd2='$ni_spd2',
index3='$ni_index3',
spd3='$ni_spd3',
poin1='$po_ni_index0',
poin2='$po_ni_index1',
poin3='$po_ni_index2',
poin4='$po_ni_index3'
");
/**/

$kt1_index0 = $_POST['kt1_index0'];
$kt1_spd0 = $_POST['kt1_spd0'];
$po_kt1_index0 = $_POST['po_kt1_index0'];
$kt1_index1 = $_POST['kt1_index1'];
$kt1_spd1 = $_POST['kt1_spd1'];
$po_kt1_index1 = $_POST['po_kt1_index1'];
$kt1_index2 = $_POST['kt1_index2'];
$kt1_spd2 = $_POST['kt1_spd2'];
$po_kt1_index2 = $_POST['po_kt1_index2'];
$kt1_index3 = $_POST['kt1_index3'];
$kt1_spd3 = $_POST['kt1_spd3'];
$po_kt1_index3 = $_POST['po_kt1_index3'];

mysql_query("

INSERT INTO wawan_nilkat1 SET no_beasiswa = '$no_bea', 
index0='$kt1_index0',
spd0='$kt1_spd0',
index1='$kt1_index1',
spd1='$kt1_spd1',
index2='$kt1_index2',
spd2='$kt1_spd2',
index3='$kt1_index3',
spd3='$kt1_spd3',
poin1='$po_kt1_index0',
poin2='$po_kt1_index1',
poin3='$po_kt1_index2',
poin4='$po_kt1_index3'
");

$kt2_index0 = $_POST['kt2_index0'];
$kt2_spd0 = $_POST['kt2_spd0'];
$po_kt2_index0 = $_POST['po_kt2_index0'];
$kt2_index1 = $_POST['kt2_index1'];
$kt2_spd1 = $_POST['kt2_spd1'];
$po_kt2_index1 = $_POST['po_kt2_index1'];
$kt2_index2 = $_POST['kt2_index2'];
$kt2_spd2 = $_POST['kt2_spd2'];
$po_kt2_index2 = $_POST['po_kt2_index2'];
$kt2_index3 = $_POST['kt2_index3'];
$kt2_spd3 = $_POST['kt2_spd3'];
$po_kt2_index3 = $_POST['po_kt2_index3'];

mysql_query("

INSERT INTO wawan_nilkat2 SET no_beasiswa = '$no_bea', 
index0='$kt2_index0',
spd0='$kt2_spd0',
index1='$kt2_index1',
spd1='$kt2_spd1',
index2='$kt2_index2',
spd2='$kt2_spd2',
index3='$kt2_index3',
spd3='$kt2_spd3',
poin1='$po_kt2_index0',
poin2='$po_kt2_index1',
poin3='$po_kt2_index2',
poin4='$po_kt2_index3'
");

$kt3_index0 = $_POST['kt3_index0'];
$kt3_spd0 = $_POST['kt3_spd0'];
$po_kt3_index0 = $_POST['po_kt3_index0'];
$kt3_index1 = $_POST['kt3_index1'];
$kt3_spd1 = $_POST['kt3_spd1'];
$po_kt3_index1 = $_POST['po_kt3_index1'];
$kt3_index2 = $_POST['kt3_index2'];
$kt3_spd2 = $_POST['kt3_spd2'];
$po_kt3_index2 = $_POST['po_kt3_index2'];
$kt3_index3 = $_POST['kt3_index3'];
$kt3_spd3 = $_POST['kt3_spd3'];
$po_kt3_index3 = $_POST['po_kt3_index3'];
mysql_query("

INSERT INTO wawan_nilkat3 SET no_beasiswa = '$no_bea', 
index0='$kt3_index0',
spd0='$kt3_spd0',
index1='$kt3_index1',
spd1='$kt3_spd1',
index2='$kt3_index2',
spd2='$kt3_spd2',
index3='$kt3_index3',
spd3='$kt3_spd3',
poin1='$po_kt3_index0',
poin2='$po_kt3_index1',
poin3='$po_kt3_index2',
poin4='$po_kt3_index3'
");

$kt4_index0 = $_POST['kt4_index0'];
$kt4_spd0 = $_POST['kt4_spd0'];
$po_kt4_index0 = $_POST['po_kt4_index0'];
$kt4_index1 = $_POST['kt4_index1'];
$kt4_spd1 = $_POST['kt4_spd1'];
$po_kt4_index1 = $_POST['po_kt4_index1'];
$kt4_index2 = $_POST['kt4_index2'];
$kt4_spd2 = $_POST['kt4_spd2'];
$po_kt4_index2 = $_POST['po_kt4_index2'];
$kt4_index3 = $_POST['kt4_index3'];
$kt4_spd3 = $_POST['kt4_spd3'];
$po_kt4_index3 = $_POST['po_kt4_index3'];
mysql_query("

INSERT INTO wawan_nilkat4 SET no_beasiswa = '$no_bea', 
index0='$kt4_index0',
spd0='$kt4_spd0',
index1='$kt4_index1',
spd1='$kt4_spd1',
index2='$kt4_index2',
spd2='$kt4_spd2',
index3='$kt4_index3',
spd3='$kt4_spd3',
poin1='$po_kt4_index0',
poin2='$po_kt4_index1',
poin3='$po_kt4_index2',
poin4='$po_kt4_index3'
");

$kt5_index0 = $_POST['kt5_index0'];
$kt5_spd0 = $_POST['kt5_spd0'];
$po_kt5_index0 = $_POST['po_kt5_index0'];
$kt5_index1 = $_POST['kt5_index1'];
$kt5_spd1 = $_POST['kt5_spd1'];
$po_kt5_index1 = $_POST['po_kt5_index1'];
$kt5_index2 = $_POST['kt5_index2'];
$kt5_spd2 = $_POST['kt5_spd2'];
$po_kt5_index2 = $_POST['po_kt5_index2'];
$kt5_index3 = $_POST['kt5_index3'];
$kt5_spd3 = $_POST['kt5_spd3'];
$po_kt5_index3 = $_POST['po_kt5_index3'];
mysql_query("

INSERT INTO wawan_nilkat5 SET no_beasiswa = '$no_bea', 
index0='$kt5_index0',
spd0='$kt5_spd0',
index1='$kt5_index1',
spd1='$kt5_spd1',
index2='$kt5_index2',
spd2='$kt5_spd2',
index3='$kt5_index3',
spd3='$kt5_spd3',
poin1='$po_kt5_index0',
poin2='$po_kt5_index1',
poin3='$po_kt5_index2',
poin4='$po_kt5_index3'
");

echo ("<SCRIPT LANGUAGE='JavaScript'>
    window.alert('Data Berhasil Ditambahkan')
    window.location.href='index.php?conten=input_bea';
    </SCRIPT>");
?>