...
$di_daf = trim($di_daf);
if(isset($_POST['no_cari'])){
        $no_cari = $_POST['no_cari'];
    }else{
        $no_cari ='';
    }
    if(isset($_POST['tahun_cari'])){
        $tahun_cari = $_POST['tahun_cari'];
    }else{
        $tahun_cari ='';
    }
$sql = mysql_query(
...
a.st_wawan ='Y' AND
b.nin = c.nin AND
AND a.no_beasiswa = '".$no_cari."' AND YEAR(a.tgl_daftar) = '".$tahun_cari."'
");
...
$object->getActiveSheet()->setTitle('Data Penerima Beasiswa');
...
$date = date('Ymd H:i:s');
header('Content-Disposition: attachment;filename="Data Hasil Wawancara-'.$date.'.xls"'); 
